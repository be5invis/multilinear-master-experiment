...
export function regularizeMultiLinearMasterDim(md: MultiLinearMasterDim): MultiLinearMasterDim {
	...
	let minX = scaleUp(1),
		minY: MultiLinearStop = { left: 1, right: 1, inclusive: 1 },
		maxX = scaleUp(-1),
		maxY: MultiLinearStop = { left: 1, right: 1, inclusive: 0 };
	...
	if (minX > scaleUp(-1)) {
		stops.set(scaleUp(-1), { left: minY.left, right: minY.left, inclusive: 1 });
	}
	if (maxX < scaleUp(+1)) {
		stops.set(scaleUp(+1), {
			left: maxY.right,
	...
	if (!stops.has(0)) {
		...
		stops.set(0, { left: y0, right: y0, inclusive: 0 });
	}
	return {
		axis: md.axis,
		points: Array.from(stops).sort(byX).map(scaleDown),
